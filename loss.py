...
def info_nce_loss(anchor_emb, pos_emb, neg_emb, weith, temperature, return_logits=False):
    ...
    loss = F.cross_entropy(logits / temperature, label,reduction='none')  
    loss = (loss * weith).sum()/weith.sum()
