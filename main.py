...
def train_one_step(batch, emb_loader, loader, model:BaselineModel):
    ...
    return loss, neg_sim, pos_sim, top1_correct, top10_correct, entropy,neg_id.shape[0],logits.shape[0]
...
if __name__ == '__main__':
    ...
    for epoch in range(1, const.num_epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            loss, neg_sim, pos_sim, top1_correct, top10_correct, entropy,num_neg,num_sample = train_one_step(batch, emb_loader, neg_loader, model,)
            ...
            top10_correct_sum += top10_correct
            entropy_sum += entropy
            pos_sim_sum += pos_sim
            neg_sim_sum += neg_sim
            global_step += 1
        ...
        top1_correct_sum /= len(train_loader)
        top10_correct_sum /= len(train_loader)
        entropy_sum /= len(train_loader)
        pos_sim_sum /= len(train_loader)
        neg_sim_sum /= len(train_loader)
        save_dir = Path(os.environ.get('TRAIN_CKPT_PATH'), f"epoch={epoch}_global_step={global_step}.training_loss={loss_sum:.4f}.top1_correct={top1_correct_sum:.4f}.top10_correct={top10_correct_sum:.4f}.entropy={entropy_sum:.4f}.pos_sim={pos_sim_sum:.4f}.neg_sim={neg_sim_sum:.4f}")
