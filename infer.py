...
def infer():
    ...
    def predict(user_id, user_feat, feat):
        _, item_id, item_feat, context_feat = feat
        item_feat = emb_loader.add_mm_emb(item_id, item_feat)
        item_id = item_id.to(const.device)
        item_feat = to_device(item_feat)
        context_feat = to_device(context_feat)
        with torch.amp.autocast(device_type=const.device, dtype=torch.bfloat16):
    ...
    for str_user_id, user_id, user_feat, item_feat1,item_feat2, item_feat3 in dataloader:
        user_id,user_feat = user_id.to(const.device), to_device(user_feat)
        next_token_emb1 = predict(user_id, user_feat, item_feat1)
        next_token_emb2 = predict(user_id, user_feat, item_feat2)
        next_token_emb3 = predict(user_id, user_feat, item_feat3)
        
        next_token_emb = torch.mean(torch.stack([next_token_emb1, next_token_emb2, next_token_emb3], dim=0), dim=0)
        sim12 = torch.sum(next_token_emb1 * next_token_emb2, dim=-1)
        sim13 = torch.sum(next_token_emb1 * next_token_emb3, dim=-1)
