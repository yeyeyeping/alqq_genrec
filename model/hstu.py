...
class PointwiseAggregatedAttention(nn.Module):
    def __init__(self, d_model, 
        ...
        self.rab_p = RelativeAttentionBias(num_heads, 
                                           ...
                                           relative_attention_bucket_dim,
                                           relative_attention_max_distance)
        self.rab_t = RelativeTimeIntervalBias(max_time_interval, time_interval_dim, num_heads)
    
    def split_heads(self, x, batch_size):
        x = x.view(batch_size, -1, self.num_heads, self.head_dim)
        return x.permute(0, 2, 1, 3)

    def forward(self, v, k, q, time_matrix, mask):
        ...
        rab = self.rab_p(q.shape[2], k.shape[2], device=q.device) + self.rab_t(time_matrix)

        att_w_bias = F.silu(attention_scores + rab)
...
class HSTUBlock(nn.Module):
    ...
    def forward(self, x, time_matrix, attn_mask):
        # Pointwise Projection
        x_proj = F.silu(self.f1(x))
        u, v, q, k = self.split(x_proj)

        # Spatial Aggregation
        av = self.pointwise_attn(v, k, q, time_matrix, attn_mask)
    ...
    def forward(self, seqs, time_matrix, attention_mask):
        for i in range(len(self.attention_layers)):
            if self.norm_first:
                ...
                mha_outputs = self.attention_layers[i](x, time_matrix, attention_mask)
...
#                  relative_attention_max_distance):
#         super().__init__()
#         self.layers = nn.ModuleList([HSTUBlock(d_model, num_heads, max_time_interval, time_interval_dim, relative_attention_num_buckets, relative_attention_bucket_dim, relative_attention_max_distance) for _ in range(num_layers)])

#     def forward(self, x, time_matrix, attn_mask):
#         for layer in self.layers:
#             x = layer(x, time_matrix, attn_mask) + x
#         return x
